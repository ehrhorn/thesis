...
Measurements of certain neutrino properties may help us understand phenomena such as why the universe is matter-dominated~\autocite{Nunokawa2008} and dark matter~\autocite{Baur2019}.
...
At some point an interaction will create such a particle (\vref{sec:neutrino_interactions}), which oscillates between different flavors (\vref{sec:neutrino_oscillations}).
...
When the data arrives for analysis, different level of cleaning takes place to root out noise (\vref{sec:levels}).
This pipeline also attempts to classify event types, using decision trees, until finally algorithms---none of them machine learning based---reconstruct neutrino properties of interest such as energy and direction (\vref{sec:current_reconstruction_methods}), and the events at all levels of cleaning are saved in an IceCube proprietary format, called I3 (\vref{sec:i3_file_format}).
...
The opponent algorithm is detailed (\vref{sec:opponents}), before the structure of the data itself used in training is explained (\vref{sec:data_structure}) and the algorithm laid out (\vref{sec:algorithm}).
