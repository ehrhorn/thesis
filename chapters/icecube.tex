...
\begin{figure}
     ...
     \end{subfigure}
        \caption{\Vref{fig:icecube_detector} shows an artist rendition of the IceCube detector as a scale comparison to the Eiffel Tower. \Vref{fig:icecube_event_1} shows a neutrino event in IceCube. This is a high energy track-like event, for which angular reconstruction is possible even by eye.}\label{fig:icecube}
\end{figure}
...
They are essentially in-ice, high quality digital cameras, capable of detecting single photons~\cite{Aartsen2017}.
...
\todo[inline]{Expound on digitizers.}
...
It should be noted that the DOMs are pretty reliable; by 2016 87 DOMs were not operational~\cite{Aartsen2017}; this is rather fortuitous, inasmuch as they are non-repairable (at least they're impossible to retrieve from the ice).
...
\begin{figure}
     ...
     \end{subfigure}
        \caption{\Vref{fig:dom_pic} shows a picture of a DOM,~\vref{fig:dom_schematic}~\cite{Abbasi2009} shows a schematic of a DOM.}\label{fig:doms}
\end{figure}
...
This reconstruction has a median angular resolution (the arc-distance between reconstruction and the true track) on Monte Carlo (MC,~\vref{sec:monte_carlo_simulation}) data of \( \theta_{\text{med}} = \SI{4.211}{\degree} \)~\cite{Aartsen2014}.
...
Relative to L3, \SI{99}{\percent} of pure noise and over \SI{94}{\percent} of atmospheric muons events are removed, while \SI{80}{\percent} of neutrinos are kept, resulting in a \( 100 : 10 : 1 \) (muon : neutrino : noise) ratio.
...
An event can be contained within an I3Frame, and will contain information such as pulses and their associated DOMs, indexed by keys, which can then be looked up in a complementary GCD file, containing the geometry of the detector.
...
This is a minor annoyance, as one can build this in relatively short time, but it does not to be compiled and is not available easily e.g.\ via PyPi (and with good reasons, as it contains many dependencies).
...
\begin{figure}
     ...
     \end{subfigure}
        ...
        Both images are courtesy IceCube,~\vref{fig:deepcore_string_pattern} is from \url{https://icecube.wisc.edu/gallery/view/140}.}\label{fig:string_patterns}
\end{figure}
...
\todo[inline]{Hmm, need more about DeepCore. But what???}
