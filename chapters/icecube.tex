...
The files basically consist of so-called I3Frames, which may hold either primitives (such as ints, floats, etc.) or objects.
The I3Frames are serialised, and so anything contained within them must be serialisable.
...
This is unusable for machine learning applications, where it is a requirement that your sets are defined in advance, \textit{and} are easily retrievable on demand, as your training will probably include randomisation each epoch.

This was addressed in this project by using SQLite; see \vref{sec:sqlite}.

\section{DeepCore}\label{sec:deepcore}

Before all of IceCube's strings were even deployed---a laborious task only possible during the short austral summer months---it was recognised that the energy range of the detector might not be sufficient for certain analysis purposes.
