...
\begin{figure}
     ...
     \end{subfigure}
        \caption{(a), from~\cite{Ahlers2018}, shows the types of rays relevant to IceCube, defining up- and down-going muons. Neutrinos---created by atmospheric interaction with cosmic rays or in cosmic processes---interact either in the atmosphere (creating background muons), or in the Earth (creating up-going muons). (b), from ~\cite{Spiering2012}, shows the energy spectrum of neutrinos from different sources. Atmospheric neutrinos dominate in the energy region supported by IceCube.}\label{fig:rays_and_spectrum}
\end{figure}
...
An electromagnetic cascade (or \enquote{shower}) comes from a high energy electron suffering bremsstrahlung, radiating a photon which creates an electron-positron pair.
This pair production again experiences bremsstrahlung, and the process continues until the energy is spent.
Hadronic cascades result from the nuclear interaction of hadrons (composite particles made up of two or more quarks) and the concept of color confinement which disallows a quark from existing alone; it requires less energy to create a companion quark from the void.
...
\begin{itemize}
	...
	\item A tau lepton will live very shortly (on the order of \SI{1e-13}{\second}) owing to its massive nature, and decays again either hadronically (creating a cascade) or leptonically (creating neutrinos and a muon or an electron, with about equal probability)
	\item Muons are special: they do not decay immediately, as taus, and are less affected by the processes stopping electrons. They are thus able to leave long tracks in the detector, a track-like signature simply because their dominant energy loss mechanism is ionisation, whilst electrons also lose energy to bremsstrahlung
\end{itemize}
...
Different conditions leads to different payloads sent from the DOM to the surface; if the local coincidence is triggered, both the fADC and the ATWD waveforms are sent.
...
The servers at the IceCube Lab perform first a quick reconstruction, \enquote{linefit}~\cite{Aartsen2014}---a minimizer using a Huber penalty---used as a seed for the \enquote{Single-Photo-Electron-Fit} (SPE), a maximum likelihood estimation~\cite{Ahrens2004}.
...
IceCube has two sources of unwanted noise, when detecting neutrinos: background atmospheric muons produced by cosmic rays, which trigger the detector at a rate of around \SI{3000}{\kilo\hertz}~\cite{Aartsen2016b}, and noise produced directly in the DOMs.
This noise has two components, thermal and non-thermal~\cite{Blot2020}.
The thermal variety stems from spontaneous emission of single electrons from the photocathodes, while the non-thermal comes from radioactive decays in the DOM glass and sees a rate of \SIrange{500}{800}{\hertz}~\cite{Kelley2014}.
...
After all cuts, at the final level, between \SI{22}{\percent} and \SI{36}{\percent} simulated neutrinos remain, depending on current type and flavor.
In the case of charged current muon neutrinos, at level 2 the rate is \SI{6.16}{\milli\hertz}, while at level 5 it is \SI{1.39}{\milli\hertz}, with a neutrino efficiency of \SI{22.6}{\percent}~\cite{Blot2020}.
...
Muons are identified by looking for light in the upper regions of the detector, while noise is found by looking at hit timing or number of hits.
Several variables are derived and cut on; examples include \verb|NchCleaned| (number of hit DOMs in the cleaned pulses series) cut at \( \geq 6 \)) to identify noise, \verb|VertexGuess Z| (\( z \)-coordinate of the first pulse in the cleaned pulses series) cut at \( < \SI{-120}{\meter} \) to identify muons and \verb|DCFiducialHits| (the number of hit DeepCore fiducial DOMs in the cleaned pulses series) cut at \( > 2 \) to identify noise.
...
After L5, the rates are \SI{2.16}{\milli\hertz} for \textit{all} neutrinos, \SI{0.93}{\milli\hertz} for atmospheric muons and \SI{0.07}{\milli\hertz} for pure noise.
...
\begin{figure}
    ...
    \caption{Reconstruction comparisons between PegLeg (the previous reconstruction algorithm) and RetroReco (the current).
    Reconstructions are measured in seconds per event.
    From~\cite{Blot2020}.}\label{fig:reco_comp}
\end{figure}
...
\begin{figure}
     ...
     \end{subfigure}
        \caption{(a), from~\cite{IcecubeDOM1}, shows the string pattern of IceCube and DeepCore, (b), from~\cite{Ishihara2019}, shows the proposed upgrade.}\label{fig:string_patterns}
\end{figure}
...
This data was used to determine the optimal placement of DeepCore, around \SIrange{2100}{2450}{\meter}, in the clearest ice measured, escaping the antarctic dust layer, left behind some \num{65000} years ago~\cite{Abbasi2012}, which sees a significant uptick in scattering.
DeepCore comprises eight new strings, with an average inter-string horizontal spacing of \SI{72}{\meter}, and is logically joined with seven near IceCube strings such that the two types on the whole comprise the DeepCore detector; some strings are placed only \SI{42}{\meter} apart.
Each new DeepCore string has \num{50} DOMs below \SI{2100}{\meter}, with the remaining \num{10} above the dust layer.
