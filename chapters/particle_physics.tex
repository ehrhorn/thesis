...
This was solved by the electroweak theory, introducing bosons that mediate the interaction over small distances after symmetry breaking via the Higgs mechanism; the \( \textup{W} \) and \( \textup{Z} \) bosons.
...
Furthermore, two of the bosons contain electric charge (the \( \textup{W}^{\pm} \) bosons), while the last one is electrically neutral (the \( \textup{Z}^{0} \) boson), leading to the phenomenon of charged- and neutral currents.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{./images/particle_physics/Beta_spectrum_of_RaE.jpg}
    \caption{Energy spectrum of \( {}^{210}\textup{B} \) beta decay, with \( E_{\text{max}} = \SI{1.16}{\mega\electronvolt} \).
...
By absorbing a \( \textup{W} \) boson, an electron may turn into an electron neutrino, or---as in the case of beta decay, \vref{fig:beta_decay}---a down quark may convert into an up quark, changing a neurino into a proton, with the resulting \( \textup{W}^{-} \) boson decaying into an electron and antielectron neutrino, conserving charge and lepton number.
...
Nicola Cabibbo developed a framework for explaining this discrepancy, wherein so-called weak eigenstates are not the same as mass eigenstates.
For example, both down- and strange quarks may decay into an up quark, giving a weak eigenstate
\begin{equation}
	\textup{d}^{\prime} = V_{\textup{ud}} \textup{d} + V_{\textup{us}} \textup{s},
\end{equation}
with \( \left| V_{\textup{ud}} \right|^2 \) the probability of a down quark decaying, and \( \left| V_{\textup{us}} \right|^2 \) the probability of a strange quark decaying.
...
\begin{equation}
	...
	=
	\begin{bmatrix}
		V_{\textup{ud}} & V_{\textup{us}} \\
		V_{\textup{cd}} & V_{\textup{cs}}
	\end{bmatrix}
	\begin{bmatrix}
		\textup{d} \\
		\textup{s}
	...
	=
	\begin{bmatrix}
		\cos{\theta_{\textup{c}}} & \sin{\theta_{\textup{c}}} \\
		-\sin{\theta_{\textup{c}}} & \cos{\theta_{\textup{c}}}
	\end{bmatrix}
	\begin{bmatrix}
		\textup{d} \\
		\textup{s}
	\end{bmatrix},
\end{equation}
with \( \theta_{\textup{c}} \) the Cabibbo angle.

This concept was extended to neutrino oscillations.
This Nobel-winning idea was used to explain the apparent failure of the Homestake experiment, also known as the solar neutrino problem.

The Sun fuses protons in the p-p chain, wherein two protons fuse to create deuterium, a positron, an electron neutrino and excess energy:
\begin{equation}
	\textup{p} + \textup{p} \rightarrow {}_{1}^{2}\textup{D} + \textup{e}^{+} + \nu_{\textup{e}} + \SI{1.442}{\mega\electronvolt}.
\end{equation}
Though other interactions occur, the Standard Solar Model predicts that the Sun only produces electron neutrinos, with the flux predicted to be on the order of \( \SI{7e10}{\per\centi\meter\squared\per\second} \).
...
\begin{equation}
	\nu_{\textup{e}} + {}^{37}\textup{Cl} \rightarrow {}^{37}\textup{Ar} + \textup{e}^{-}.
\end{equation}
...
This, of course, can have many causes: the calculations may be wrong, the experimental setup may be faulty or the solar model may be wrong.
Raymond Davis, the experiment-runner, however, stood his ground, and some 40-odd years later the rather shocking fact about neutrinos was confirmed, and validated the experiment: neutrinos change flavour during propagation.
