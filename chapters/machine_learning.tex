...
\enquote{Convolutional neural networks} create feature maps by employing discrete convolutions via kernels that \enquote{slide} over the input and aggregate information in the receptive field.
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.7\textwidth]{./images/machine_learning/convolution.png}
    \caption{An example of discrete convolutions. The dark blue \( 3 \times 3 \) kernel slides over the input, and sums the product of the weights and inputs (an inner product in all but name) forming a feature map (green matrix) in the process.
