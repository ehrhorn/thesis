...
\todo{Need a little more on classic CNN used}
...
\todo[inline]{Loss functions\ldots what clever thing to say? From Troels: Lots! Difference between Loss for classification and regression, and MAE vs. RMS for regression. Mention the problems with loss for multiple regression, with three solutions (make model for each output, transforming each term to contribute similarly, or transforming all variables), and which you chose and why. Also the problem with angular Loss (vanishing gradient), and that different loss function trainings can be combined in ensemble.}
